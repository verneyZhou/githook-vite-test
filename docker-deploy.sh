...
###### docker 部署脚本 v2 ######
echo -e "---------docker Login Aliyun--------"
docker login --username=$1 registry.cn-beijing.aliyuncs.com -p $2

echo -e "---------docker Pull--------"
docker pull registry.cn-beijing.aliyuncs.com/zhou_aliyun_image/githook-vite-test:v1.0

# 如果已经有同名的容器，删除掉
if [ "$(docker ps -aq -f name=githook-aliyun-container-v2)" ]; then
  echo 'docker rm ...'
  docker stop githook-aliyun-container-v2  # 停止容器
  docker rm githook-aliyun-container-v2  # 删除容器
fi
